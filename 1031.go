...
func getSequceSum(A []int, n int) int {
	max := -1
	for i := 0; i+n <= len(A); i++ {
		sum := 0
		for j := i; j < i+n; j++ {
...
func maxSumTwoNoOverlap(A []int, L int, M int) int {
	res := 0
	for i := 0; i+L <= len(A); i++ {
		res1 := getSequceSum(A[:i], M)
		res2 := getSequceSum(A[i+L:], M)
		sum := 0
		for j := i; j < i+L; j++ {
			sum += A[j]
		}
		fmt.Println(sum, res1, res2)
		if res1 > res2 {
			sum += res1
		} else {
			sum += res2
...
func main() {
	fmt.Println(maxSumTwoNoOverlap([]int{1,0,1}, 1, 1))
}
