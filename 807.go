...
func maxIncreaseKeepingSkyline(grid [][]int) int {
	...
	for i := 0; i < w; i++ {
		...
		lineMax = append(lineMax, lMax)
	}
	res := 0
	for i := 0; i < w; i++ {
		for j := 0; j < h; j++ {
			res += intMin(rowMax[i], lineMax[j]) - grid[i][j]
		}
	}
	return res
}
...
func main() {
	numbers := [][]int{{3, 0, 8, 4}, {2, 4, 5, 7}, {9, 2, 6, 3}, {0, 3, 1, 0}}
	fmt.Println(maxIncreaseKeepingSkyline(numbers))
}