	...
	for i := 0; i < len(board); i++ {
		for j := 0; j < len(board[0]); j++ {
			if string(board[i][j]) == "R" {
				for x := i; i >= 0; x-- {
					if string(board[x][j]) == "B" {
						break
					} else if string(board[x][j]) == "p" {
						res++
						break
					}
				}
				for y := i; y < 8; y++ {
					if string(board[y][j]) == "B" {
						break
					} else if string(board[y][j]) == "p" {
						res++
						break
					}
				}
				for k := j; k >= 0; k-- {
					if string(board[i][k]) == "B" {
						break
					} else if string(board[i][k]) == "p" {
						res++
						break
					}
				}
